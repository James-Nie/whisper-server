...
export class FriendsService {
    ...
    async getAllFriendsIds(userId: User['user_id']): Promise<Array<User['user_id']>> {
        ...
        friendshipMess.forEach(friendshipMes => {
            if(friendshipMes.user_id === userId) {
                friendsIds.push(friendshipMes.friend_user_id);
            }
            else friendsIds.push(friendshipMes.user_id);
        })
        return friendsIds;
    }
    ...
    //         })) {
    //             // 获取目标用户的socketID
    //             const targetSocket = await this.socketService.getSocketMes({
    //                 _id: targetUserSocketId
    //             })
    //             const {socket_id: targetSocketId} = targetSocket;
    //             // 向目标用户立即发送好友申请通知
