...
import { jwtConfig } from "../common/config/config";
...
export class AuthService {
    ...
    async validateToken(accessToken: string): Promise<boolean> {
        ...
        if (await this.userService.checkUserExistence({user_id: decodedToken.userId})) {
            return true;
        } else {
            throw new HttpException("无效Token，此为非法登录",HttpStatus.FORBIDDEN);
        }
    ...
    async validatePassword(userName: string, password: string): Promise<boolean> {
        return await this.userService.checkUserExistence({ user_name: userName }) &&
            await this.userService.checkUserPassword(userName, password);
    }
